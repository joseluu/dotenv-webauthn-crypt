...
Public-key recovery & disambiguation: the master key is `SHA256(pubkey)`, and the pubkey is recovered from a fresh assertion signature each load (never stored). ECDSA recovery returns ~2 candidates per signature. To select the right one robustly — even with randomized signatures / bumped signCount (USB FIDO2 keys) — `init` stores `PUBKEY_TAG = SHA256(SHA256(pubkey))` (domain-separated from the master key, so it leaks neither the pubkey nor the master key). `_recover_public_key` selects by tag when available, else falls back to the legacy `Y_PARITY` bit (only reliable for deterministic authenticators like Windows Hello). The tag is written to both the credential file and the `.env` recovery header, making vaults self-sufficient/portable; `load_dotenv` prefers the header tag.
